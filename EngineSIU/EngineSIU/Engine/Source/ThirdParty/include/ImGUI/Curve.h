...
int Curve(const char* label, const ImVec2& size, const int maxpoints, ImVec2* points, int* selection, const ImVec2& rangeMin = ImVec2(0, 0), const ImVec2& rangeMax = ImVec2(1, 1))
{
    ...
    if (hovered && hoveredPoint != -1 && IsMouseDown(0) && currentSelection != -1)
    {
        SelectedPoint = hoveredPoint;
    }

    if (SelectedPoint >= 0 && SelectedPoint < pointCount)
    {
        float position[2] = { points[SelectedPoint].x, points[SelectedPoint].y };
        if (ImGui::DragFloat2(("Point " + std::to_string(SelectedPoint)).c_str(), position, 0.001))
        {
            points[SelectedPoint] = ImVec2(position[0], position[1]);

            const float pointXRangeMin = (SelectedPoint > 0) ? points[SelectedPoint - 1].x : rangeMin.x;
            const float pointXRangeMax = (SelectedPoint + 1 < pointCount) ? points[SelectedPoint + 1].x : rangeMax.x;
            
            points[SelectedPoint] = ImClamp(points[SelectedPoint], ImVec2(pointXRangeMin, rangeMin.y), ImVec2(pointXRangeMax, rangeMax.y));
        }
    }
